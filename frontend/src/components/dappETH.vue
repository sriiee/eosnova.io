  ...
  <div id="dappstation">
              ...
              <v-menu bottom offset-y>
                <template v-slot:activator="{ on }">
                  <v-btn
                    v-on="on"
                  >
                    <img src="https://s3.ap-northeast-2.amazonaws.com/eosnova.io.resource/image2.0/dappstation/dapp-icon-eth.png">
                  </v-btn>
                </template>
                <v-list>
                  <v-list-tile
    ...
    <div id="contents">
      ...
      <ul class="dappList" v-if="!computed">
        ...
        -->
        <li v-for="item in ethDappList" class="list">
          <a :href="item.url.android.length > 0 ? item.url.android : item.url.ios" target="_blank">
            ...
            <p class="chain">
              <img src="https://s3.ap-northeast-2.amazonaws.com/eosnova.io.resource/image2.0/dappstation/dapp-cate-eth.png" width="10"/>
...
<script>
  ...
  export default {
      ...
      let ethDApp = async () => {
        await axios.get('/api/ethDapp')
          .then(res => {
            ...
            this.hideData = this.ethDappList[0]['title'];

            let cateList = [];
            let dappLen = this.ethDappList.length;
            for( let i=dappLen-1; i>-1; i-- ){
              // category
              cateList.push(this.ethDappList[i]['category'][0]);
              cateList.push(this.ethDappList[i]['category'][1]);

              // url account 치환
              if (this.ethDappList[i]['url']['android'].match('{account}')) {
                let url = this.ethDappList[i]['url']['android'],
                  urlRes = url.replace('{account}', 'suchagoodday');
                this.ethDappList[i]['url']['android'] = urlRes;
              }
              if (this.ethDappList[i]['url']['ios'].match('{account}')) {
                let url = this.ethDappList[i]['url']['ios'],
                  urlRes = url.replace('{account}', 'suchagoodday');
                this.ethDappList[i]['url']['ios'] = urlRes;
              }

              // enable
              if( !this.ethDappList[i]['enable']['android'] && !this.ethDappList[i]['enable']['ios'] ){
                this.ethDappList.splice(i,1)
              }
            }
    ...
    computed: {
      dappRst: function(){
        ...
        if(this.search){
          this.loading = true;
          if( category !== 'all' ){
            return vm.ethDappList.filter((dapp)=> {
              this.loading = false;
    ...
    mounted(){
      document.getElementById('ETH').classList.add('on');
      if( langPr === 'en') {
