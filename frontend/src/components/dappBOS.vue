              ...
              <v-menu bottom offset-y>
                <template v-slot:activator="{ on }">
                  <v-btn
                    v-on="on"
                  >
                    <img src="https://s3.ap-northeast-2.amazonaws.com/eosnova.io.resource/image2.0/dappstation/dapp-icon-bos.png">
                  </v-btn>
                </template>
                <v-list>
                  <v-list-tile
      ...
      <ul class="dappList" v-if="!computed">
        ...
        <li v-for="item in dappList" class="list">
          <a :href="item.url.android.length > 0 ? item.url.android : item.url.ios" target="_blank">
            ...
            <p class="chain">
              <img src="https://s3.ap-northeast-2.amazonaws.com/eosnova.io.resource/image2.0/dappstation/dapp-cate-bos.png" width="10"/>
...
<script>
  ...
  export default {
    ...
    created(){
      let eosDApp = async () => {
        await axios.get('/api/bosDapp')
          .then(res => {
            for (let key in res['data']) {
              res['data'][key]['chain'] = 'BOS';
            }
    ...
    mounted(){
      document.getElementById('BOS').classList.add('on');
      if( langPr === 'en') {
